...
## v6.5.4 - 아바타 음성 관리 개선
- [x] AI 얼굴 갤러리에서 아바타별 매칭된 음성 확인 및 바로 변경 기능 (FaceGallery 상세 다이얼로그에 음성 변경 UI 추가)
- [x] 클론 음성이 없는 사용자를 위해 음성 선택 드롭다운에 '음성 클론 만들기' 안내 링크 추가 (Step1 아바타 음성 선택에 링크 추가)
- [x] 강의 포맷 선택 시 프로젝트 설정에 저장하여 다음 접속 시에도 유지 (formatSelection JSON 컬럼 추가, createProject/updateProject에서 저장)

## v6.5.5 - 포맷 변경/즐겨찾기 음성/원클릭 적용
- [x] 프로젝트 편집 화면에서 저장된 포맷을 시각적으로 확인하고 변경할 수 있는 "포맷 설정 변경" 버튼 추가
- [x] AI 얼굴 갤러리에서 즐겨찾기한 얼굴에 기본 음성을 미리 설정해두는 기능
- [x] 음성 클론 생성 완료 후 자동으로 최근 프로젝트의 아바타에 매칭하는 원클릭 적용 기능
- [x] 모든 새 UI 텍스트 20개 언어 번역
## v6.5.6 - 포맷 경고/자동 음성 적용/개별 선택
- [x] 포맷 변경 시 기존 슬라이드/스크립트에 영향을 미치는 경우 경고 메시지 표시
- [x] 즐겨찾기 아바타의 기본 음성이 새 프로젝트 생성 시 자동으로 적용되는 연동 기능
- [x] 음성 클론 적용 시 개별 아바타 선택 UI (현재 전체 적용만 → 개별 선택 가능)
- [x] 모든 새 UI 텍스트 20개 언어 번역
## v6.5.7 - 스크립트 재배치/역할별 음성/적용 이력
- [x] 포맷 변경 후 기존 스크립트를 새 포맷에 맞게 자동 재배치하는 마이그레이션 기능
...
## UX 개선 v6.6.1
- [x] 일반 스크립트 생성(Step 2)에 동일한 로딩 UI 패턴 적용
- [x] 아바타 카드 얼굴 사진 변경 시 fade-in 미리보기 애니메이션
- [x] PPT 스크립트 생성 완료 후 자동으로 Step 4 이동 가이드 토스트
- [x] AI 스크립트 생성 후 '스크립트 저장' 버튼 추가 - 각 슬라이드 스크립트를 DB에 저장

## UX 개선 v6.6.6
- [x] 저장 완료 후 자동으로 매칭 에디터(Step 4)로 이동
- [x] AI 스크립트 생성 시 기존 내용 덮어쓰기 확인 다이얼로그
- [x] 스크립트 저장 후 총 예상 발표 시간 요약 표시
...
## v6.9.0 영상 이력 페이지 기능 강화
- [x] 재생성 버튼 - 동일 설정으로 영상 다시 생성
- [x] 영상 썸네일 자동 캡처 및 표시
- [x] 프로젝트 제목/생성 날짜 기준 검색 기능

## v6.9.1 버그 수정
- [x] 영상 생성 완료 후 결과 화면(미리보기+다운로드)으로 자동 이동
- [x] 영상 이력 페이지에서 완료 상태인데 "대기 중" 표시 + 버튼 미노출 수정
- [x] 미리보기 단계에 "생성된 영상 확인하기" 버튼 추가
- [x] 프로덕션 DB를 로컬 MySQL로 독립 운영 + Manus DB 데이터 마이그레이션 완료

## v6.10.0 영상 이력 강화 + DB 백업
...
## v6.14.0 프로그레스 바 애니메이션 + 결과 공유 버튼
- [ ] 영상 생성 프로그레스 바에 로딩 애니메이션 추가 (진행 상태 명확히 표시)
- [ ] 생성된 영상 결과 페이지에 공유 버튼 추가 (소셜 미디어/링크로 바로 공유)
- [ ] 프로덕션 배포
